    ...
    public HashMap<Integer, PLSQLCopGuideline> getGuidelines() {
        try {
            if ((this.guidelines == null)) {
                ...
                final List<ComposedChecks> annotations = ListExtensions.<Annotation, ComposedChecks>map(
                        IterableExtensions.<Annotation>toList(IterableExtensions.<Annotation>filter(
                                ((Iterable<Annotation>) Conversions.doWrapArray(this.getClass().getAnnotations())),
                                _function)),
                        _function_1);
                    ...
                    for (final Class<? extends AbstractDeclarativeValidator> validator : _validators) {
                        {
                            ...
                            Collection<PLSQLCopGuideline> _values = ((PLSQLCopValidator) validatorInstance)
                                    .getGuidelines().values();
